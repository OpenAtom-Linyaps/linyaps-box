...
static const MountFlag mount_flags[] = { { MS_RDONLY, "MS_RDONLY" },
                                         { MS_NOSUID, "MS_NOSUID" },
                                         { MS_NODEV, "MS_NODEV" },
                                         { MS_NOEXEC, "MS_NOEXEC" },
                                                ...
                                                const linyaps_box::config::mount_t &mount)
{
    ...
    std::optional<remount_t> processCgroupMount(const linyaps_box::container &container,
                                                const linyaps_box::utils::file_descriptor &root,
                                                const linyaps_box::config::mount_t &mount,
                                                bool &is_sys_rbind)
    {
    ...
    setupMountDestination(const linyaps_box::container &container,
                          const linyaps_box::utils::file_descriptor &root,
                          const linyaps_box::config::mount_t &mount,
                          bool &is_sys_rbind)
    {
    ...
    bool needsRemount(const linyaps_box::config::mount_t &mount)
    {
    ...
    unsigned long computeRemountFlags(const linyaps_box::config::mount_t &mount)
    {
        unsigned long remount_flags = mount.flags | MS_REMOUNT;
        if (mount.type != "proc") {
            remount_flags |= MS_BIND;
        }
        return remount_flags;
    }

    std::optional<remount_t> finalizeRemount(linyaps_box::utils::file_descriptor destination_fd,
                                             const linyaps_box::config::mount_t &mount,
                                             unsigned long remount_flags)
    {
        auto delay_readonly_mount =
                remount_t{ std::move(destination_fd), remount_flags, mount.data };
                                                    ...
                                                    const linyaps_box::config::mount_t &mount)
    {
        ...
        std::optional<remount_t> result;
        if (mount.type.rfind("cgroup", 0) != std::string::npos) {
            result = processCgroupMount(container, root, mount, is_sys_rbind);
        } else {
            auto destination_fd = setupMountDestination(container, root, mount, is_sys_rbind);
            do_propagation_mount(destination_fd, mount.propagation_flags);

            if (!needsRemount(mount)) {
                LINYAPS_BOX_DEBUG() << "no need to remount";
            } else {
                auto remount_flags = computeRemountFlags(mount);
                result = finalizeRemount(std::move(destination_fd), mount, remount_flags);
            }
        }
...
// Helper functions for configure_device
namespace {
static const char *dump_mode(mode_t mode)
{
    if (S_ISCHR(mode)) {
        return "Character";
    }
...
static bool is_existing_device_valid(const std::filesystem::path &destination,
                                     mode_t mode,
                                     int type,
                                     dev_t dev,
                                     uid_t uid,
...
{
    auto stat = linyaps_box::utils::lstatat(fd, "");
    bool valid = true;

    if (__S_ISTYPE(stat.st_mode, type)) {
        LINYAPS_BOX_DEBUG() << "the type of existing device: " << destination
                            << " is not required\n"
    ...
    if ((stat.st_mode & ~S_IFMT) != (mode & ~S_IFMT)) {
        valid = false;
    }
                      ...
                      gid_t gid)
{
    ...
    if (destination_fd.has_value()) {
        if (is_existing_device_valid(destination, mode, type, dev, uid, gid, *destination_fd)) {
            done = true;
        }
    }
    ...
    if (!done) {
        ...
        this->mkdirs(parent, 0755, true);
        if (destination_fd.has_value()) {
            this->unlinkat(*destination_fd, "", 0);
        }
        this->mknodat(root, destination.relative_path(), mode | type, dev);
        this->chmod(destination, mode);
        this->chown(destination, uid, gid);
    }
}
                                     ...
                                     try {
                                         ...
                                         auto new_dev =
                                                 linyaps_box::utils::open_at(root, path, O_PATH);
                                     ...
                                     void configure_default_devices()
                                     {
                                         ...
                                         this->configure_device("/dev/null",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(1, 3),
                                                                uid,
                                                                gid);
                                         this->configure_device("/dev/zero",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(1, 5),
                                                                uid,
                                                                gid);
                                         this->configure_device("/dev/full",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(1, 7),
                                                                uid,
                                                                gid);
                                         this->configure_device("/dev/random",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(1, 8),
                                                                uid,
                                                                gid);
                                         this->configure_device("/dev/urandom",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(1, 9),
                                                                uid,
                                                                gid);
                                         this->configure_device("/dev/tty",
                                                                default_mode,
                                                                S_IFCHR,
                                                                makedev(5, 0),
                                                                uid,
                                     ...
                                     void configure_dev_symlinks()
                                     {
                                         ...
                                         linyaps_box::utils::symlink_at("/proc/self/fd",
                                                                        dev_fd,
                                                                        "fd");
                                         linyaps_box::utils::symlink_at("/proc/self/fd/0",
                                                                        dev_fd,
                                                                        "stdin");
                                         linyaps_box::utils::symlink_at("/proc/self/fd/1",
                                                                        dev_fd,
                                                                        "stdout");
                                         linyaps_box::utils::symlink_at("/proc/self/fd/2",
                                                                        dev_fd,
                                                                        "stderr");
                                     }
                                     }
                                     ...
                                     [[noreturn]] void
                                     execute_process(const linyaps_box::config::process_t &process)
                                     {
                                         ...
                                         if (process.user.additional_gids) {
                                             ret = setgroups(process.user.additional_gids->size(),
                                                             process.user.additional_gids->data());
                                             if (ret != 0) {
                                                 throw std::system_error(errno,
                                                                         std::generic_category(),
                                                                         "setgroups");
                                     ...
                                     void wait_prestart_hooks_result(
                                             const linyaps_box::container &container,
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         ...
                                         throw unexpected_sync_message(
                                                 sync_message::PRESTART_HOOKS_EXECUTED,
                                                 message);
                                     }

                                     void wait_create_runtime_result(
                                             const linyaps_box::container &container,
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         ...
                                         LINYAPS_BOX_DEBUG()
                                                 << "Request execute createRuntime hooks";

                                         socket << std::byte(
                                                 sync_message::REQUEST_CREATERUNTIME_HOOKS);
                                         ...
                                         auto message = sync_message(byte);
                                         if (message
                                             == sync_message::CREATERUNTIME_HOOKS_EXECUTED) {
                                             ...
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         ...
                                         socket << std::byte(
                                                 sync_message::CREATECONTAINER_HOOKS_EXECUTED);
                                     ...
                                     void do_pivot_root(const std::filesystem::path &rootfs)
                                     {
                                         ...
                                         auto old_root = linyaps_box::utils::open(
                                                 "/",
                                                 O_DIRECTORY | O_PATH | O_CLOEXEC);
                                         auto new_root = linyaps_box::utils::open(
                                                 rootfs.c_str(),
                                         ...
                                         LINYAPS_BOX_DEBUG()
                                                 << "Pivot root old root: "
                                                 << dump_mount_flags(old_root_stat.f_flags);

                                         auto new_root_stat = linyaps_box::utils::statfs(new_root);
                                         LINYAPS_BOX_DEBUG()
                                                 << "Pivot root new root: "
                                                 << dump_mount_flags(new_root_stat.f_flags);
                                     ...
                                     void set_capabilities(const linyaps_box::container &container,
                                                           int last_cap)
                                     {
#ifdef LINYAPS_BOX_ENABLE_CAP
                                         LINYAPS_BOX_DEBUG() << "Set capabilities";
                                         const auto &capabilities =
                                                 container.get_config().process.capabilities;

                                         if (last_cap <= 0) {
                                             throw std::runtime_error(
                                                     "kernel does not support capabilities");
                                         }
                                         ...
                                         std::unique_ptr<_cap_struct, decltype(&cap_free)> caps(
                                                 cap_init(),
                                                 cap_free);
                                         int ret{ -1 };
                                         ...
                                         ret = setresuid(process.user.uid,
                                                         process.user.uid,
                                                         process.user.uid);
                                         if (ret < 0) {
                                             throw std::system_error(errno,
                                         ...
                                         ret = setresgid(process.user.gid,
                                                         process.user.gid,
                                                         process.user.gid);
                                         if (ret < 0) {
                                             throw std::system_error(errno,
                                             ...
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         ...
                                         socket << std::byte(
                                                 sync_message::STARTCONTAINER_HOOKS_EXECUTED);
                                     ...
                                     void close_other_fds(std::set<uint> except_fds)
                                     {
                                         ...
                                         for (auto fd = except_fds.begin();
                                              std::next(fd) != except_fds.end();
                                              ++fd) {
                                             auto low = *fd + 1;
                                     ...
                                     try {
                                         if (getenv("LINYAPS_BOX_CONTAINER_PROCESS_TRACE_ME")
                                             != nullptr) {
                                             ...
                                             LINYAPS_BOX_INFO()
                                                     << "OCI runtime in container namespace "
                                                        "waiting for signal USR1 to continue";
                                             pause();
                                         ...
                                         LINYAPS_BOX_DEBUG()
                                                 << "OCI runtime in container namespace starts";
                                         ...
                                         if (rootfs.is_relative()) {
                                             rootfs = std::filesystem::canonical(
                                                     container.get_bundle() / rootfs);
                                         }

                                         initialize_container(container.get_config(), socket);
                                         auto [runtime_cap] =
                                                 get_runtime_security_status(); // get runtime
                                                                                // status before
                                                                                // pivot root
                                     ...
                                     // NOTE: All function in this namespace are running in the
                                     // runtime namespace.
                                                     ...
                                                     &namespaces)
                                     {
                                         ...
                                         int flag = SIGCHLD;
                                         LINYAPS_BOX_DEBUG()
                                                 << "Add SIGCHLD, flag=0x" << std::hex << flag;
                                         if (!namespaces) {
                                             return flag;
                                         }
                                         ...
                                         for (const auto &ns : *namespaces) {
                                             ...
                                             case linyaps_box::config::linux_t::namespace_t::IPC: {
                                                 flag |= CLONE_NEWIPC;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWIPC, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::UTS: {
                                                 flag |= CLONE_NEWUTS;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWUTS, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::
                                                     MOUNT: {
                                                 flag |= CLONE_NEWNS;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWNS, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::PID: {
                                                 flag |= CLONE_NEWPID;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWPID, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::NET: {
                                                 flag |= CLONE_NEWNET;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWNET, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::USER: {
                                                 flag |= CLONE_NEWUSER;
                                                 LINYAPS_BOX_DEBUG() << "Add CLONE_NEWUSER, flag=0x"
                                                                     << std::hex << flag;
                                             } break;
                                             case linyaps_box::config::linux_t::namespace_t::
                                                     CGROUP: {
                                     ...
                                     void
                                     set_rlimits(const linyaps_box::config::process_t::rlimits_t
                                                         &rlimits)
                                     {
                                         std::for_each(
                                                 rlimits.begin(),
                                                 rlimits.end(),
                                                 [](const linyaps_box::config::process_t::rlimit_t
                                                            &rlimit) {
                                     ...
                                     std::tuple<int, linyaps_box::utils::file_descriptor>
                                     start_container_process(
                                             const linyaps_box::container &container,
                                             const linyaps_box::config::process_t &process)
                                     {
                                         LINYAPS_BOX_DEBUG() << "All opened file describers before "
                                                                "socketpair:\n"
                                                             << linyaps_box::utils::inspect_fds();
                                         auto sockets = linyaps_box::utils::socketpair(
                                                 AF_UNIX,
                                                 SOCK_SEQPACKET | SOCK_CLOEXEC,
                                                 0);
                                         LINYAPS_BOX_DEBUG()
                                                 << "All opened file describers after socketpair:\n"
                                                 << linyaps_box::utils::inspect_fds();
                                         const auto &config = container.get_config();

                                         // config rlimits before we enter new user namespace
                                         if (const auto &rlimits = config.process.rlimits;
                                             rlimits) {
                                         ...
                                         int clone_flag =
                                                 runtime_ns::generate_clone_flag(namespaces);
                                         clone_fn_args args = { &container,
                                                                &process,
                                                                std::move(sockets.second) };

                                         LINYAPS_BOX_DEBUG()
                                                 << "OCI runtime in runtime namespace: PID="
                                                 << getpid() << " PIDNS="
                                                 << linyaps_box::utils::get_pid_namespace();

                                         child_stack stack;
                                         int child_pid = clone(container_ns::clone_fn,
                                                               stack.top(),
                                                               clone_flag,
                                                               (void *)&args);
                                         if (child_pid < 0) {
                                         ...
                                         if (child_pid == 0) {
                                             throw std::logic_error(
                                                     "clone should not return in child");
                                         }

                                         LINYAPS_BOX_DEBUG()
                                                 << "OCI runtime in container namespace: PID="
                                                 << child_pid << " PIDNS="
                                                 << linyaps_box::utils::get_pid_namespace(
                                                            child_pid);
                                     ...
                                     [[nodiscard]] int execute_user_namespace_helper(
                                             const std::vector<std::string> &args)
                                     {
                                         ...
                                         if (WIFSIGNALED(status)) {
                                             throw std::runtime_error(
                                                     "user_namespace helper exited which caused by "
                                                     "signal "
                                                     + std::to_string(WTERMSIG(status)));
                                         }

                                         throw std::runtime_error(
                                                 "user_namespace helper exited abnormally");
                                     }

                                     void set_deny_groups(const linyaps_box::container &container,
                                                          const std::filesystem::path &filepath)
                                     {
                                         ...
                                         auto file = linyaps_box::utils::open(
                                                 filepath,
                                     ...
                                     void configure_gid_mapping(
                                             pid_t pid, const linyaps_box::container &container)
                                     {
                                         ...
                                         auto self_process = std::filesystem::path{ "/proc" }
                                                 / std::to_string(pid);
                                         const auto is_single_mapping =
                                                 (gid_mappings_v.size() == 1
                                                  && gid_mappings_v[0].size == 1
                                                  && gid_mappings_v[0].host_id
                                                          == gid_mappings_v[0].container_id);
                                         if (is_single_mapping) {
                                             const auto &data =
                                                     linyaps_box::get_private_data(container);
                                             if (!data.deny_setgroups) {
                                             ...
                                             pid_t pid, const linyaps_box::container &container)
                                     {
                                         ...
                                         // If we only need to mapping a single and equivalent uids,
                                         // we could write it directly. This condition is the most
                                         // of our usage, so try it at first instead of newuidmap
                                         ...
                                         auto self_process = std::filesystem::path{ "/proc" }
                                                 / std::to_string(pid);
                                         const auto is_single_mapping =
                                                 (uid_mappings_v.size() == 1
                                                  && uid_mappings_v[0].size == 1
                                                  && uid_mappings_v[0].host_id
                                                          == uid_mappings_v[0].container_id);
                                         if (is_single_mapping) {
                                             ...
                                             // NOTE: The writing process must have the same
                                             // effective user ID as the process that created the
                                             // user namespace
                                             throw std::system_error{ errno,
                                                                      std::system_category(),
                                                                      "single uid mapping failed" };
                                         }
                                     ...
                                     void configure_container_cgroup(
                                             [[maybe_unused]] const linyaps_box::container
                                                     &container)
                                     {
                                     ...
                                     void configure_container_namespaces(
                                             const linyaps_box::container &container,
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         LINYAPS_BOX_DEBUG()
                                                 << "Waiting OCI runtime in container namespace to "
                                                    "request configure namespace";
                                         ...
                                         socket >> byte;
                                         {
                                             auto message = sync_message(byte);
                                             if (message
                                                 != sync_message::REQUEST_CONFIGURE_NAMESPACE) {
                                                 throw unexpected_sync_message(
                                                         sync_message::REQUEST_CONFIGURE_NAMESPACE,
                                                         message);
                                             }
                                         ...
                                         if (linux) {
                                             ...
                                             if (namespaces) {
                                                 if (std::find_if(
                                                             namespaces->cbegin(),
                                                             namespaces->cend(),
                                                             [](const linyaps_box::config::linux_t::
                                                                        namespace_t &ns) -> bool {
                                                                 return ns.type
                                                                         == linyaps_box::config::
                                                                                 linux_t::namespace_t::
                                                     ...
                                                     // TODO: if not mapping a range of uid/gid, we
                                                     // could set uid/gid in the container process
                                                     if (const auto &uid_mappings =
                                                                 linux->uid_mappings;
                                                         uid_mappings) {
                                                         configure_uid_mapping(pid, container);
                                                     }

                                                     if (const auto &gid_mappings =
                                                                 linux->gid_mappings;
                                                         gid_mappings) {
                                     ...
                                     void
                                     prestart_hooks(const linyaps_box::container &container,
                                                    linyaps_box::utils::file_descriptor &socket)
                                     {
                                         if (!container.get_config().hooks.prestart) {
                                             return;
                                         ...
                                         LINYAPS_BOX_DEBUG()
                                                 << "Waiting request to execute prestart hooks";
                                         ...
                                         if (message != sync_message::REQUEST_PRESTART_HOOKS) {
                                             throw unexpected_sync_message(
                                                     sync_message::REQUEST_PRESTART_HOOKS,
                                                     message);
                                         }
                                         ...
                                         for (const auto &hook :
                                              container.get_config().hooks.prestart.value()) {
                                             execute_hook(hook);
                                         }
                                             ...
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         ...
                                         LINYAPS_BOX_DEBUG() << "Waiting request to execute create "
                                                                "runtime hooks";
                                         ...
                                         for (const auto &hook :
                                              container.get_config().hooks.create_runtime.value()) {
                                             execute_hook(hook);
                                         }
                                     ...
                                     void wait_create_container_result(
                                             const linyaps_box::container &container,
                                             linyaps_box::utils::file_descriptor &socket)
                                     {
                                         if (!container.get_config().hooks.create_container) {
                                         ...
                                         LINYAPS_BOX_DEBUG()
                                                 << "Waiting OCI runtime in container namespace "
                                                    "send create container hooks result";
                                         ...
                                         if (message
                                             == sync_message::CREATECONTAINER_HOOKS_EXECUTED) {
                                             LINYAPS_BOX_DEBUG()
                                                     << "Create container hooks executed";
                                             return;
                                         }
                                         throw unexpected_sync_message(
                                                 sync_message::CREATECONTAINER_HOOKS_EXECUTED,
                                     ...
                                     void
                                     wait_socket_close(linyaps_box::utils::file_descriptor &socket)
                                     try {
                                         LINYAPS_BOX_DEBUG() << "All opened file describers:\n"
                                                             << linyaps_box::utils::inspect_fds();
                                         std::byte byte;
                                         ...
                                         socket >> byte;
                                     } catch (const linyaps_box::utils::
                                                      file_descriptor_closed_exception &e) {
                                     ...
                                     void poststart_hooks(const linyaps_box::container &container)
                                     {
                                         ...
                                         for (const auto &hook :
                                              container.get_config().hooks.poststart.value()) {
                                             execute_hook(hook);
                                         }
                                     }

                                     void poststop_hooks(
                                             const linyaps_box::container &container) noexcept
                                     {
                                     ...
                                     linyaps_box::container::container(
                                             const status_directory &status_dir,
                                             const std::string &id,
                                         ...
                                         , data(new linyaps_box::container_data)
                                     {
...
#endif
                                         {
                                             ...
                                             status.PID = getpid();
                                             status.status =
                                                     container_status_t::runtime_status::CREATING;
                                             status.bundle = bundle;
                                             status.created = std::to_string(
                                         ...
                                         case cgroup_manager_t::disabled: {
                                             this->manager =
                                                     std::make_unique<disabled_cgroup_manager>();
                                         } break;
                                     ...
                                     const linyaps_box::config &
                                     linyaps_box::container::get_config() const
                                     {
                                         return this->config;
                                     }

                                     const std::filesystem::path &
                                     linyaps_box::container::get_bundle() const
                                     {
                                         return this->bundle;
                                     }

                                     // maybe we need a internal run function?
                                     int linyaps_box::container::run(
                                             const config::process_t &process) const
                                     {
                                         int container_process_exit_code{ -1 };
                                         try {
                                             // TODO: there are some thing that should be done
                                             // before starting the container process e.g. do
                                             // something before creating cgroup by selecting
                                             // manager, selinux label, seccomp setup, etc.

                                             // TODO: cgroup preenter
                                             auto [child_pid, socket] =
                                                     runtime_ns::start_container_process(*this,
                                                                                         process);

                                             {
                                                 auto status = this->status();
                                                 assert(status.status
                                                        == container_status_t::runtime_status::
                                                                CREATING);
                                                 status.PID = child_pid;
                                                 status.status = container_status_t::
                                                         runtime_status::CREATED;
                                                 this->status_dir().write(status);
                                             }

                                             runtime_ns::configure_container_namespaces(*this,
                                                                                        socket);
                                             runtime_ns::prestart_hooks(*this, socket);
                                             runtime_ns::create_runtime_hooks(*this, socket);
                                             runtime_ns::wait_create_container_result(*this,
                                                                                      socket);
                                             ...
                                             container_process_exit_code =
                                                     runtime_ns::wait_container_process(
                                                             this->status().PID);
                                     ...
                                     void linyaps_box::container::cgroup_preenter(
                                             const cgroup_options &options,
